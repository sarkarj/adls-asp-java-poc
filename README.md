...
Phase 8 always updates all 6 secrets. Do not skip.
...
curl -X POST \
  ...
  -d "Written from App Service — $(date)"
...
╚══════════════════════════════════════════╝
File      : output.txt
Content   : Written from App Service — ...
```
...
POST /write?file=output.txt  Body: "file content"
    │
    ▼
AdlsController.writeFile("output.txt", content)
    │  @PostMapping — POST only, consumes text/plain
    │  @RequestParam — 400 if missing
    │  @RequestBody — Spring enforces 10MB limit
    ▼
AdlsService.writeFile("output.txt", content)
    │  validateFileName() → regex allowlist + null byte check
    │  fileClient.create(overwrite=true)
...
│   ├── AdlsService.java                 ← @Service — readFile() + writeFile()
...
| Stack traces in responses | ✅ Never — @ExceptionHandler |
